...
import { formatINR, PAID_TOUR_CARDS } from '../data/paidTourContent';
...
const regions = ['All', ...Array.from(new Set(allPackages.map((tour) => tour.region)))];
const types = ['All', ...Array.from(new Set(allPackages.map((tour) => tour.type)))];
const sortOptions = ['Featured', 'Price: Low to High', 'Price: High to Low', 'Rating', 'Duration'];

export const PackagesPage: React.FC<PackagesPageProps> = ({ onStartBooking, onNavigate }) => {
  useScrollReveal();
      ...
      <section className="py-8 pb-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filtered.map((pkg) => (
              <div key={pkg.id} className="group bg-white rounded-[1.75rem] overflow-hidden shadow-lg border border-orange-100/40 card-lift cursor-pointer" onClick={() => onStartBooking(pkg.id)}>
                <div className="relative h-64 overflow-hidden img-zoom">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); toggleLike(pkg.id); }}
                    className={`absolute top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center shadow-lg z-10 transition-all cursor-pointer ${liked.has(pkg.id) ? 'bg-rose-500 text-white' : 'bg-white/90 text-slate-600 hover:text-rose-500'}`}>
                    <Heart className={`w-4 h-4 ${liked.has(pkg.id) ? 'fill-current' : ''}`} />
                  </button>
                  <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-28 h-14 bg-white rounded-t-full flex items-end justify-center pb-3 z-10 shadow-lg">
                    <div className="flex gap-px text-brand">{[...Array(5)].map((_, j) => <Star key={j} className="w-3 h-3 fill-current" />)}</div>
                  </div>
                  <div className="absolute bottom-3 right-4 z-10">
                    <span className="glass-white text-slate-800 text-[10px] font-bold px-2 py-1 rounded-full flex items-center gap-1">
                      <Star className="w-3 h-3 text-amber-500 fill-current" />{pkg.rating.toFixed(1)}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-6 space-y-3">
                  ...
                  <div className="flex items-center gap-4 text-xs text-slate-400 font-medium pt-1">
                    <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" />{pkg.durationText}</span>
                    <span className="flex items-center gap-1"><Users className="w-3.5 h-3.5" />{pkg.groupText}</span>
                    <span className="flex items-center gap-1"><Star className="w-3.5 h-3.5 text-amber-500 fill-current" />{pkg.reviews.toLocaleString('en-IN')}</span>
                  </div>
