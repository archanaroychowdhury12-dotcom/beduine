      ...
      {cycleState === 'Result Published' && (
        <div className="pt-6 border-t border-slate-100 space-y-6">
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-5 gap-3 items-end">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 outline-none text-slate-700 bg-white font-bold"
                >
                  {bulkPlan === 'Silver Plan' && ['Sundarbans', 'Digha', 'Mousuni Island', 'Purulia'].map(d => <option key={d} value={d}>{d}</option>)}
                  {bulkPlan === 'Gold Plan' && ['Darjeeling', 'Dooars', 'Puri', 'Daring Bari'].map(d => <option key={d} value={d}>{d}</option>)}
          ...
          <div className="space-y-3 text-left font-sans">
            <h4 className="text-xs font-black text-slate-805 uppercase tracking-wider">Weekly Winners Table ({selectedWinners.length})</h4>
