...
export function DemoBanner({ currentUser }: { currentUser: any }) {
  ...
  if (!isDemoWalletEnabled) return null;
  ...
  // If logged in and NOT demo/admin user, do not show the banner at all
  if (currentUser && !isDemoOrAdminUser) return null;
  ...
  const handleReset = async () => {
    if (!currentUser) return;
    if (!window.confirm("Are you sure you want to reset your demo account? This will reset your balance to ₹0 and deactivate active plans.")) return;
    setLoading(true);
    setFeedback(null);
    try {
