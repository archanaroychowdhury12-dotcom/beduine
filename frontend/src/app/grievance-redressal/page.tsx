...
export default function GrievanceRedressalPage({ onNavigate }: GrievancePageProps) {
  const policy = LEGAL_POLICIES.find(p => p.slug === 'grievance-redressal');

  if (!policy) {
    return (
      <div className="p-8 text-center text-red-500 font-mono">
        Error: Grievance Redressal policy definition not found.
      </div>
    );
  }
