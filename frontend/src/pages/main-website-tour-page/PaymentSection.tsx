...
import { AdvancePaymentPlan, BookingPaymentType, PaymentDetails, TourPackage, Traveler, PickupInfo, Voucher, PriceCalculation } from '../../types';
...
interface PaymentSectionProps {
  ...
  setBookingType: (value: BookingPaymentType) => void;
  instantBookingRequired: boolean;
  setInstantBookingRequired: (value: boolean) => void;
    ...
    <div className="space-y-8 text-left">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="p-4 bg-slate-50 rounded-xl border border-slate-200/60 flex justify-between items-start gap-4">
            <div>
              ...
              <span className="text-[10px] text-emerald-700 font-semibold mt-0.5 block">Available Departure</span>
            </div>
            <button
              onClick={() => onJumpToStep(1)}
              className="text-[10px] font-black text-amber-600 hover:text-amber-700 flex items-center gap-1 shrink-0 cursor-pointer"
        ...
        <div className="bg-slate-900 text-white rounded-xl p-5 space-y-4">
          ...
          {paymentPlan.instantBookingCharge > 0 && (
            <div className="flex justify-between text-amber-300 font-bold">
              <span>Instant Booking Service Charge</span>
              <span className="font-mono font-bold">{formatINR(paymentPlan.instantBookingCharge)}</span>
            </div>
          )}

          <div className="pt-3.5 border-t border-slate-800 flex justify-between items-baseline">
            <span className="text-sm font-black uppercase text-white font-serif-premium">Total Tour Cost</span>
            <span className="text-2xl font-black text-amber-400 font-mono font-serif-premium">
              {formatINR(paymentPlan.grandTotal)}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-amber-200/80 rounded-2xl p-5 shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div>
            <span className="text-[10px] font-extrabold text-amber-600 uppercase tracking-widest">Advance Payment Method</span>
            <h4 className="text-sm font-black text-slate-900 mt-1">{paymentPlan.bookingTypeLabel}</h4>
            <p className="text-[11px] text-slate-500 mt-0.5">{paymentPlan.paymentSummary}</p>
          </div>
          <div className="grid grid-cols-2 gap-2 text-[10px] font-black uppercase">
            {[
              { id: 'fixed_departure' as const, label: 'Fixed Group' },
              { id: 'customized_tailor_made' as const, label: 'Customized' },
            ].map((item) => (
              <button
                key={item.id}
                type="button"
        ...
        <label className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 border border-slate-200 text-xs cursor-pointer">
          <input
            type="checkbox"
            checked={instantBookingRequired}
            onChange={(e) => setInstantBookingRequired(e.target.checked)}
            className="mt-0.5 h-4 w-4 text-amber-500 rounded border-slate-300 focus:ring-amber-500"
          />
          <span>
            <span className="block font-black text-slate-900">Need instant confirmation?</span>
            <span className="block text-[11px] text-slate-500 mt-0.5">
              Adds {tour.category === 'international' ? '₹5,000' : '₹2,000'} instant booking charge for urgent hotel, transport, or ticket confirmation.
            </span>
          </span>
        </label>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="rounded-2xl bg-slate-900 text-white p-4">
            <span className="text-[10px] uppercase tracking-widest text-slate-400 font-black">Payable Now</span>
            <span className="block text-2xl font-black text-amber-400 font-mono mt-1">{formatINR(paymentPlan.advanceDueNow)}</span>
            <span className="text-[11px] text-slate-300 mt-1 block">Booking confirmation advance only</span>
          </div>
          <div className="rounded-2xl bg-slate-50 border border-slate-200 p-4">
            <span className="text-[10px] uppercase tracking-widest text-slate-400 font-black">Balance Later</span>
            <span className="block text-2xl font-black text-slate-900 font-mono mt-1">{formatINR(paymentPlan.balanceDueLater)}</span>
            <span className="text-[11px] text-slate-500 mt-1 block">As per due timeline below</span>
          </div>
        </div>

        <div className="space-y-2">
          {paymentPlan.installments.map((row) => (
            <div key={row.id} className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 rounded-xl border px-3.5 py-3 text-xs ${
              row.status === 'pay_now' ? 'border-amber-300 bg-amber-50' : 'border-slate-200 bg-white'
            }`}>
              <div>
                <span className="font-black text-slate-900">{row.label} • {row.percentage}%</span>
                <span className="block text-[11px] text-slate-500 mt-0.5">{row.dueLabel}</span>
              </div>
              <span className={`font-mono font-black ${row.status === 'pay_now' ? 'text-amber-700' : 'text-slate-800'}`}>{formatINR(row.amount)}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-slate-900 text-white rounded-2xl p-5 space-y-4 shadow-md text-xs font-semibold">
        <span className="text-amber-400 font-bold block flex items-center gap-1.5">
          <ShieldAlert className="w-4 h-4" /> Payment Policy Verification & Agreements
        </span>
        ...
        <div className="space-y-3.5">
          ...
          <label className="flex items-start space-x-3 cursor-pointer select-none">
            ...
            />
            <div>
              <span className="block text-white font-bold">Acknowledge Payment & Cancellation Policies</span>
              <span className="text-[11px] text-slate-400 mt-0.5 block leading-normal">
                I agree to the BEDUINE payment schedule, cancellation charges, no-show rule, refund timeline, and 12-month credit adjustment policy.
              </span>
            </div>
          </label>
        </div>
      ...
      <div className="pt-2">
        {isProcessing ? (
          ...
          >
            ...
            <span>Pay Advance Now {formatINR(paymentPlan.advanceDueNow)}</span>
