...
export function TravelRewardSystem() {
  const phases = [
    ...
    { n: '02', icon: Bot, title: 'Random Selection Engine', accent: 'teal',
      ...
      desc: 'A secure Random Number Generator (RNG) system selects the selected ticket IDs with full transparency.',
    ...
    { n: '03', icon: FileText, title: 'Reports & Audits', accent: 'gold',
      ...
      details: ['PDF/CSV with total pool, selected IDs & timestamps', 'RNG seed logs included for verification', 'Archived for transparent internal & external audits'] },
    { n: '04', icon: Send, title: 'Winner Notification', accent: 'cyan',
      img: '/images/transparency_winner.png',
      desc: 'Selected members are notified every Sunday evening across all company channels with unique digital coupons.',
      details: ['Website Member Updates Page + Social Media posts', 'SMS & Email notification with coupon code', 'Unique QR/UUID codes - impossible to duplicate'] },
  ];
      ...
      <div className="max-w-7xl mx-auto px-5 lg:px-8 relative z-10">
        ...
        <Reveal>
          <div className="text-center mb-16 max-w-3xl mx-auto">
            <div className="inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.3em] neon-cyan font-semibold mb-4"><div className="w-8 h-px bg-cyan" /> Travel Reward Credit (TRC) Protocol <div className="w-8 h-px bg-cyan" /></div>
            ...
            <p className="mt-6 text-ink/70 text-lg leading-relaxed">Every Sunday, eligible subscribers enter a verified winner selection. The customer journey stays simple: enter, watch, and receive your result securely.</p>
          </div>
        </Reveal>
          ...
          <div className="mt-16 glass rounded-3xl p-6 lg:p-10 border border-slate-line neon-border-cyan">
            <div className="grid lg:grid-cols-2 gap-10 items-center">
              <div>
                ...
                <p className="text-sm text-ink/70 leading-relaxed mb-6">Every Sunday, subscribers log in to their BEDUINE account, use their Travel Reward Credit (TRC) to enter, and see results instantly. The selection is triggered by the system itself - not secretly by the company.</p>
                <div className="space-y-3">
                  {[
                    { step: '1', t: 'Log in on Sunday', d: 'Open your BEDUINE app or web portal' },
                    { step: '2', t: 'Activate your entry', d: 'Use your 1 Travel Reward Credit (TRC) to lock your ticket' },
                ...
                <div className="absolute inset-0 p-6 flex flex-col justify-between">
                  ...
                  <div className="glass rounded-xl p-4 border border-neon-gold/30 backdrop-blur-md">
                    <div className="text-center mb-3">
                      ...
                      <div className="text-xs text-ink/75 mt-0.5">Your entry was selected for a member travel benefit.</div>
                    </div>
  ...
  const roiCards = [
    {
      ...
      example: {
        ...
        story: 'Riya used her TRC for weekly participation. She was not selected as a winner, so one ₹500 Domestic Discount Credit unlocked for her next eligible paid tour.'
      },
    },
    {
      plan: 'Gold', price: 799, dc: 1, dcValue: 500, roi: 0,
      ...
      example: {
        ...
        story: 'Arjun used his Gold TRC in the Travel Reward/TCR cycle. As a non-winner, he received one ₹500 Domestic Discount Credit for a future paid domestic booking.'
      },
    },
    {
      plan: 'Platinum', price: 1499, dc: 1, dcValue: 500, roi: 0,
      ...
      example: {
        ...
        story: 'Priya joined the winner selection using her TRC. She was not selected, so one ₹500 Domestic Discount Credit was issued after result publication.'
      },
    },
  ];

  return (
    <section id="member-value" className="relative py-14 lg:py-20 overflow-hidden bg-transparent">
      <div className="max-w-7xl mx-auto px-5 lg:px-8 relative z-10">
        <Reveal>
          <div className="text-center mb-16 max-w-3xl mx-auto">
            <div className="inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.3em] neon-gold font-semibold mb-4"><div className="w-8 h-px bg-neon-gold" /> Non-Winner Credit Rule <div className="w-8 h-px bg-neon-gold" /></div>
            <h2 className="font-display text-4xl lg:text-6xl font-bold text-ink leading-tight">
              <KineticText text="Not selected?" />
              <br /><span className="gold-shimmer"><KineticText text="You still gain." delay={0.3} /></span>
            </h2>
            <p className="mt-6 text-ink/70 text-lg leading-relaxed">Use TRC to participate in the Travel Reward/TCR cycle. Winners receive winner benefits; eligible non-winners receive one non-cash Discount Credit for paid tour booking after result publication.</p>
          </div>
        </Reveal>
            ...
            <Reveal key={card.plan} delay={index * 0.15}>
              <div className="glass rounded-3xl overflow-hidden border border-slate-line hover:neon-border-gold transition-all duration-300 h-full flex flex-col justify-between p-6">
                <div>
                  <div className="relative h-44 rounded-2xl overflow-hidden mb-6 bg-slate-950">
                    ...
                    <div className="absolute bottom-4 left-4">
                      <div className="text-white/60 text-[10px] uppercase font-mono tracking-widest">Non-Winner Rule</div>
                      <div className="text-xl font-display font-black text-white">{card.plan} Non-Winner DC</div>
                    </div>
                  </div>

                  <h3 className="font-display text-lg font-bold text-ink mb-3">{card.plan} Non-Winner Credit Example</h3>
                ...
                <div className="rounded-xl p-3 bg-emerald-500/10 border border-emerald-500/20 flex items-center gap-2">
                  ...
                  <span className="text-xs text-emerald-400 font-semibold">Non-winner credit: ₹{card.dcValue.toLocaleString()} paid-tour discount after TRC participation</span>
                </div>
              </div>
            </Reveal>
          ))}
