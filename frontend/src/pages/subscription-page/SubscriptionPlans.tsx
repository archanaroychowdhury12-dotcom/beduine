        ...
        <div ref={cardRef} onMouseMove={onMove} onMouseEnter={() => setHovered(true)} onMouseLeave={() => { setHovered(false); onLeave(); }} className={`relative rounded-3xl overflow-hidden h-full transition-all duration-500 tilt-inner bg-gradient-to-b from-slate-950 to-slate-900 border ${
          ...
          <div className="p-7 border-b border-slate-800/80 relative overflow-visible">
            ...
            <div className="mt-4 flex items-start gap-2 rounded-xl border border-emerald-400/25 bg-emerald-400/10 p-3 text-sm font-black leading-snug text-emerald-100">
              ...
              <span>Pay ₹{plan.price.toLocaleString('en-IN')}. If you participate and remain a non-winner, use ₹{creditValue} as paid-tour Discount Credit.</span>
            </div>
          </div>
          ...
          <div className="px-7 pb-5">
            <div className="grid grid-cols-3 gap-2">
              {[{ l: 'Non-Winner DC', v: `₹ ${creditValue}` }, { l: 'Paid Tour Off', v: `Up to ${plan.paidDiscount}` }, { l: 'Name Change', v: plan.nameChange }].map((c) => (
    ...
    <Reveal delay={index * 0.1}>
      <TiltCard intensity={6}>
        <div ref={cardRef} onMouseMove={onMove} onMouseEnter={() => setHovered(true)} onMouseLeave={() => { setHovered(false); onLeave(); }} className={`relative rounded-3xl overflow-hidden h-full transition-all duration-500 tilt-inner bg-gradient-to-b from-slate-950 to-slate-900 border ${
          ...
          <div className="px-7 pb-5">
            <div className="grid grid-cols-3 gap-2">
              {[{ l: 'Non-Winner DC', v: `₹ ${creditValue}` }, { l: 'Tour Discount', v: plan.paidDiscount.startsWith('Up to') ? plan.paidDiscount : `Up to ${plan.paidDiscount}` }, { l: 'Name Change', v: plan.nameChange }].map((c) => (
                <div key={c.l} className="bg-slate-900/60 border border-slate-800/50 rounded-lg p-2.5">
                  <div className="text-[9px] uppercase tracking-widest text-slate-400 font-bold">{c.l}</div>
                  <div className="text-xs font-black text-slate-100 mt-0.5">{c.v}</div>
                </div>
          ...
          <div className="p-7 pt-0">
            ...
            <div className="text-center text-[11px] text-slate-400 font-bold mt-3 font-mono">// 12-mo validity - priority support included</div>
          </div>
        </div>
      </TiltCard>
    </Reveal>
          ...
          >
            ...
            <div className="text-left text-xs sm:text-sm leading-relaxed relative z-10 text-ink">
              ...
              <div className="text-ink/75 mt-1">
                <span className="font-semibold text-rose-700">No Cash Refunds.</span> Every active plan gets TRC eligibility. Discount Credits are issued only after Travel Reward/TCR participation when a member remains a non-winner.
              </div>
            </div>
          </motion.div>
        </Reveal>
      ...
      <div className="relative max-w-7xl mx-auto px-5 lg:px-8 z-10">
        ...
        <Reveal>
          <div className="text-center mb-16 max-w-3xl mx-auto">
            ...
            <p className="mt-6 text-ink/70 text-lg leading-relaxed">Unlock premium international destinations with priority travel support, curated tour assistance, and world-class travel experiences.</p>
          </div>
        </Reveal>
        <div className="grid md:grid-cols-3 gap-6 lg:gap-7 items-stretch">
          {INTL_PLANS.map((p, i) => (
        ...
        <Reveal>
          <div className="mt-14 glass rounded-2xl p-6 lg:p-8 border border-slate-line neon-border-cyan">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-5">
              {[{ type: 'calendar', t: '12 Months Validity', d: 'Full subscription coverage' }, { type: 'filecheck', t: 'Priority Support', d: 'End-to-end member assistance' }, { type: 'plane', t: 'Airport Lounge', d: 'Premium access included' }].map((b) => (
                <div key={b.t} className="flex items-center gap-3.5">
                  <AIIcon type={b.type} />
                  <div><div className="font-bold text-white text-sm">{b.t}</div><div className="text-xs text-slate-400 mt-0.5">{b.d}</div></div>
                </div>
