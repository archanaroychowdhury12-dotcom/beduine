...
export function SubscriptionCTA({ onSelectPlan: _onSelectPlan }: SubscriptionCTAProps) {
  return (
    <section id="contact" className="pt-12 pb-32 lg:pt-16 lg:pb-48 relative overflow-hidden">
      <div className="relative max-w-4xl mx-auto px-5 lg:px-8 text-center z-10 -translate-y-10 lg:-translate-y-16">
        <Reveal>
          ...
          <p className="mt-6 text-lg text-ink/70 max-w-2xl mx-auto font-semibold">Join thousands of travelers who trust BEDUINE for AI-curated journeys, transparent selections, and available value.</p>
          <div className="mt-10 flex flex-col sm:flex-row gap-3 justify-center items-center">
            <ParticleButton onClick={() => {
              const el = document.getElementById('plans');
              if (el) el.scrollIntoView({ behavior: 'smooth' });
            }} variant="gold" className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-full border-none px-8 py-4 text-base font-bold">
              <Crown className="w-5 h-5" /> Lock Your Entry <ChevronRight className="w-5 h-5" />
            </ParticleButton>
            <a href="tel:+918768903565" data-magnetic><ParticleButton as="span" variant="cyan" className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-full font-semibold"><Phone className="w-4 h-4" /> +91 87689 03565</ParticleButton></a>
          </div>
        </Reveal>
      </div>
    </section>
