...
interface LoginPageProps {
  onBack: () => void;
  onLoginSuccess: (user: SupabaseRawUser) => void;
...
export default function LoginPage({ onBack, onLoginSuccess, initialMode = 'login' }: LoginPageProps) {
  ...
  const handleGoogleLogin = async () => {
    ...
    if (error) {
      if (error.message.includes("provider is not enabled") || error.message.includes("Unsupported provider")) {
        ...
        notify.info(`Google sign in failed: ${error.message}`);
      }
    }
  };
  ...
  const handleFacebookLogin = async () => {
    ...
    if (error) {
      if (error.message.includes("provider is not enabled") || error.message.includes("Unsupported provider")) {
        ...
        notify.info(`Facebook sign in failed: ${error.message}`);
      }
    }
  };
  ...
  const handleCustomSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (viewMode === 'login') {
      const emailCheck = emailSchema.safeParse(customEmail);
      const passwordCheck = passwordSchema.safeParse(customPassword);
      if (!emailCheck.success) {
        notify.error(getValidationMessage(emailCheck.error));
        return;
      }
      if (!passwordCheck.success) {
        notify.error(getValidationMessage(passwordCheck.error));
        return;
      }
      ...
      if (error) {
        notify.info(`Login failed: ${error.message}`);
      } else if (data?.user) {
      ...
      if (customStep === 1) {
        ...
        const nameCheck = fullNameSchema.safeParse(customName);
        const mobileCheck = indianMobileSchema.safeParse(customMobile);
        if (!nameCheck.success) {
          notify.error(getValidationMessage(nameCheck.error));
          return;
        }
        if (!mobileCheck.success) {
          notify.error(getValidationMessage(mobileCheck.error));
          return;
        }
        if (!customCity.trim()) {
        ...
        if (error) {
          notify.info(`Sign up failed: ${error.message}`);
        } else if (data.user) {
          onLoginSuccess(data.user);
        }
      }
  ...
  const handlePhoneSubmit = async (e: React.FormEvent) => {
    ...
    if (phoneStep === 1) {
      const phoneCheck = indianMobileSchema.safeParse(phoneNumber);
      if (!phoneCheck.success) {
        notify.error(getValidationMessage(phoneCheck.error));
        return;
      }
      ...
      if (error) {
        if (error.message.includes("provider is not enabled") || error.message.includes("Unsupported provider")) {
          ...
          notify.info(`Failed to send SMS OTP: ${error.message}`);
        }
        return;
      }
      ...
      const otpCheck = otpSchema.safeParse(phoneOtp);
      if (!otpCheck.success) {
        notify.error(getValidationMessage(otpCheck.error));
        return;
      }
      ...
      if (error) {
        notify.info(`OTP Verification failed: ${error.message}`);
        return;
      }
      ...
      const phoneNameCheck = fullNameSchema.safeParse(phoneName);
      if (!phoneNameCheck.success) {
        notify.error(getValidationMessage(phoneNameCheck.error));
        return;
      }
      ...
      if (error) {
        notify.info(`Failed to save name: ${error.message}`);
      } else if (data?.user) {
        onLoginSuccess(data.user);
      }
    }
              ...
              {viewMode === 'register' ? (
                ...
                >
                  <WelcomeScreen
                    ...
                    onButtonClick={openSignupAuth}
                    secondaryActionText="Already have an account? Login Here"
                    onSecondaryActionClick={openLogin}
                  />
                </motion.div>
                      ...
                      {customStep === 1 ? (
                        <div className="space-y-4 text-left">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-slate-400 mt-1.5 font-semibold">Admin access stays on /admin-login.</p>
                          </div>
                        </div>
                      ) : (
                        <div className="space-y-4 text-left">
                          ...
                          <div>
                            <label className="block text-xs font-bold text-slate-600 mb-1.5" htmlFor="signupMobile">Mobile Number</label>
                            <div className="relative">
                              ...
                              <input
                                ...
                                className="w-full pl-11 pr-4 py-4 rounded-xl bg-white border border-slate-200/80 text-slate-800 text-sm outline-none focus:border-[#FF6B6B] transition-all focus:ring-2 focus:ring-[#FF6B6B]/10 font-mono placeholder:text-slate-400"
                              />
                            </div>
                          </div>
                          <div>
                            <label className="block text-xs font-bold text-slate-600 mb-1.5" htmlFor="signupCity">City</label>
                            <div className="relative">
