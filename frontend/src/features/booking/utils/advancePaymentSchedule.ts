...
export function calculateAdvancePaymentPlan({
  ...
  const grandTotal = Math.max(0, roundMoney(baseTourTotal + instantBookingCharge));
  const installmentBaseTotal = Math.max(0, roundMoney(baseTourTotal));

  const installments = bookingType === 'customized_tailor_made'
    ? buildInstallments(installmentBaseTotal, [
        {
  ...
  const baseAdvanceDueNow = installments.filter((row) => row.status === 'pay_now').reduce((sum, row) => sum + row.amount, 0);
  const advanceDueNow = Math.max(0, roundMoney(baseAdvanceDueNow + instantBookingCharge));
  const balanceDueLater = Math.max(0, grandTotal - advanceDueNow);
  if (instantBookingCharge > 0 && installments[0]) {
    installments[0] = {
      ...installments[0],
      amount: installments[0].amount + instantBookingCharge,
      note: `${installments[0].note || ''} Instant booking charge is payable in full with the advance.`.trim(),
    };
  }
