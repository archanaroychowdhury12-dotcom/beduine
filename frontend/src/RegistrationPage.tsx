...
type RegistrationReceipt = RegistrationFormData & {
  memberId: string;
  date: string;
  planName: string;
  planPrice: string;
...
interface RegistrationPageProps {
  ...
  onBack: () => void;
  prefilledData?: Partial<AppUser & RegistrationFormData> | null;
  onRegisterSuccess?: (user: SupabaseRawUser) => void;
...
const ALL_PLANS = [
  {
    ...
    icon: Star,
    benefits: [
      '1 TRC (Travel Reward Credit) for weekly win-entry participation',
      'Selected member tour benefit up to ₹3,000 (2N/3D)',
      '₹500 Discount Credit unlocks only if you participate and remain a non-winner',
  ...
  },
  {
    ...
    benefits: [
      ...
      '2 × ₹500 Discount Credits unlock only if you participate and remain a non-winner',
  ...
  },
  {
    ...
    benefits: [
      ...
      '4 × ₹500 Discount Credits unlock only if you participate and remain a non-winner',
  ...
  },
  {
    ...
    benefits: [
      ...
      '₹5,000 International Discount Credit unlocks only if you participate and remain a non-winner',
      'Up to 5% off on other paid tours',
      'Name change – One time allowed',
      'Priority travel support included'
    ]
  },
  {
    id: 'Gold_Int',
    ...
    benefits: [
      ...
      '2 × ₹5,000 International Discount Credits unlock only if you participate and remain a non-winner',
      'Up to 7% off on other paid tours',
      'Name change – Two times allowed',
      'Priority support + premium trip assistance'
    ]
  },
  {
    id: 'Platinum_Int',
    ...
    benefits: [
      ...
      '4 × ₹5,000 International Discount Credits unlock only if you participate and remain a non-winner',
      'Up to 10% off on other paid tours',
      'Name change – Unlimited allowed',
      'Priority international travel support',
...
export default function RegistrationPage({ initialPlanName, onBack, prefilledData, onRegisterSuccess, currentUser, onRedirectToLogin }: RegistrationPageProps) {
  ...
  const handleNextStep = () => {
    if (step === 1) {
      const personalCheck = registrationPersonalSchema.safeParse(formData);
      if (!personalCheck.success) {
        notify.error(getValidationMessage(personalCheck.error));
        return;
      }
      ...
      if (age < 18) {
        notify.info("Oops! You must be 18 years or older to register for a membership.");
        return;
      }
    } else if (step === 2) {
      const addressCheck = registrationAddressSchema.safeParse(formData);
      if (!addressCheck.success) {
        notify.error(getValidationMessage(addressCheck.error));
        return;
      }
    }
    setStep(prev => prev + 1);
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.is18Plus || !formData.agreeTerms) {
      notify.info("Please confirm you are 18+ and agree to the Terms & Conditions.");
      return;
    }
    if (!currentUser) {
      notify.info('Please create your account before completing registration.');
      onRedirectToLogin?.();
      return;
    }
    ...
    // Subscription payment/activation happens separately; no Discount Credit is issued here.
    const registrationMetadata = {
      full_name: formData.fullName,
      phone: formData.mobile,
      city: formData.city,
      pendingPlanName: selectedPlan.name,
      pendingPlanPrice: selectedPlan.price,
      pendingPlanType: selectedPlan.type as 'domestic' | 'international',
      dob: formData.dob,
    ...
    const { data, error } = await supabase.auth.updateUser({ data: registrationMetadata });
    if (error) {
      notify.error(`Registration update failed: ${error.message}`);
      return;
    }
      ...
      <section className="min-h-screen pt-28 pb-16 px-4 sm:px-6 lg:px-8 relative overflow-hidden flex flex-col items-center justify-center bg-cosmos">
        ...
        <div className="max-w-md w-full z-10 bg-white/85 backdrop-blur-2xl border border-white/50 rounded-3xl overflow-hidden shadow-[0_20px_50px_rgba(30,49,71,0.18)] relative">
          <WelcomeScreen
            ...
            description="Create your membership account to receive TRC (Travel Reward Credit) tokens for weekly win-entry participation, non-winner Discount Credits, and curated destination benefits."
              ...
              <div className="flex justify-between items-start mb-6">
                <div>
                  <div className="flex items-center gap-2">
                    <img src="/images/bedune_logo_cropped.png" alt="Logo" className="w-8 h-8 object-cover rounded-full border border-slate-200" />
                    <div>
                ...
                {step === 2 && (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-1">Nominee information is mandatory to securely assign winner benefit claim rights.</p>
                    </div>
                    ...
                    <div className="space-y-3 pt-3 border-t border-slate-200">
                      ...
                      <label className="flex items-start gap-3 cursor-pointer text-xs sm:text-sm text-slate-600 font-medium">
                        <input 
                          ...
                          required
                        />
                        <span>I agree to the Terms & Conditions and understand that travel subscriptions are non-refundable and include TRC entry eligibility; Discount Credits are issued only to eligible non-winners after draw participation.</span>
                      </label>
                    </div>
                  </motion.div>
                )}
              ...
              >
                ...
                <div className="p-3 bg-rose-50 border border-rose-100 rounded-xl">
                  <p className="text-[10px] text-slate-600 leading-relaxed font-medium">
                    <strong className="text-[#FF6B6B]">Value Protection Floor:</strong> Discount Credits unlock only after Lucky Draw/TCR participation when you are an eligible non-winner.
                  </p>
                </div>
              </div>
            </div>
