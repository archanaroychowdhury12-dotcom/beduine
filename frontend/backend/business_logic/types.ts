...
export interface PlanData {
  name: string;
  price: number;
  /** Backwards-compatible alias for non-winner Discount Credit count. */
  credits: number;
  /** TRC/Lucky Draw entries issued on verified subscription checkout. Always 1 in the current model. */
  trcCredits: number;
  /** Discount Credits unlocked only after verified non-winner result. */
